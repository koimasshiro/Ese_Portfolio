...
import { navlogo, menu, close } from "../assets";
    ...
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-primary`}>
      <div className="w-full flex justify-between items-center max-w-7xl mx-auto">
        <Link to='/' className="flex items-center gap-2" onClick={()=> {setActive(""); window.scrollTo(0, 0);}}>
          <img src={navlogo} alt="logo" className="w-20 h-20 object-contain"/>
          <p className="text-white text-[18px] font-bold flex cursor-pointer">Eseosa<span className="sm:block hidden"> Imhariagbe</span></p>
        </Link>
